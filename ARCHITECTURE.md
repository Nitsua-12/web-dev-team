...
- `agents/website_demo`: 34 tests (`test_deploy.py`, `test_generate_demo.py`, `test_photo_palette.py`) — the demo-hosting/noindex pure functions (`build_root_robots_txt()`, `find_indexable_html_outside_allowlist()`, `read_indexable_slugs()`, the per-page robots meta tag token, `write_sitemap()`), plus full `httpx.MockTransport`-based coverage of the photo/color-matching pipeline (`get_palette()` end-to-end, each stage's failure modes, the accent-color-selection thresholds) added once the underlying Places Photos access issue was confirmed resolved.
...
- Photo/color personalization — **resolved 2026-08-16.** The Places `photos` field had been silently empty for months (200 OK, no photos, even for landmarks with heavy coverage), suspected to be an account-side Photos SKU/enablement gap rather than a code bug. Re-tested against 5 real pilot leads and the field is now populated for all of them with no code changes on the request side, confirming the account-side theory. All 9 live demos regenerated and redeployed with real per-lead accent colors. `photo_palette.py` gained full test coverage (`test_photo_palette.py`, `httpx.MockTransport`-based) plus two hardening fixes surfaced while writing those tests: a malformed-JSON Details response and an undecodable photo both used to raise uncaught exceptions that would crash the whole `generate_demo.py --force` batch rather than just skip that lead's palette — same failure class already fixed once in `psi_client.py`, now fixed here too. See [`agents/website_demo/README.md`](agents/website_demo/README.md#color-palette-real-but-never-the-real-photo).

### Production roadmap — not yet built

1. **Build sending, gated behind approval.** Email via a transactional provider; SMS only after real legal review of the TCPA exposure flagged in §13. This is the highest-stakes remaining piece and should be built last and deliberately.
2. **Move orchestration off manual CLI for good.** `run_pipeline.py` (§6) replaced running four scripts by hand with one command — the remaining step is Temporal (or a lighter scheduler) once runs need to happen unattended or on a schedule rather than triggered by a person.
3. **Add per-agent structured logging and basic monitoring** per §9 — the pipeline-level logging exists now; instrumentation inside each agent (not just print statements) is still open, worth doing once there's unattended volume to watch.
4. **Move secrets to a real secrets manager**, add DB backups, per §11.
5. **Scale Discovery beyond the pilot** with explicit per-run cost budgets, informed by real spend data from the first full 25-metro batch.
...
| ~~Photo/color-matching feature broken~~ | **Resolved 2026-08-16** — was an account-side Places Photos gap, not a code bug; confirmed fixed against 5 real leads, all demos regenerated with real colors | — |
...
- Real *photo* personalization (not just derived colors) once there's a clear legal answer on using a business's own public photos in a pitch built for them — the Places Photos access issue itself is resolved (§16), this is now purely a policy question, not a technical one
...
| Website Demo Generation | **Built**, including live photo-derived color personalization (§16). |
